...
import { SplitTrafficNodeSchema } from "./schema"

export default function SplitTrafficNodeViewer({ data, id }: { data: SplitTrafficNodeSchema, id: string | number }) {
    ...
    <>
      ...
      <Card className="w-72 hover:border-blue-500" onMouseOver={() => onOpenToolbar(true)} onMouseOut={() => onOpenToolbar(false)}>
        ...
        <CardContent className="p-4 pt-0 flex flex-col gap-2">
          {
            data.blocks.map((blockItem) => <SplitTrafficBlockViewer key={blockItem.id} data={blockItem} />)
          }
        </CardContent>
      </Card>
    </>
