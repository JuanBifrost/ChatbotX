...
import { CopyIcon, EyeIcon, FingerprintIcon, LinkIcon, PlayIcon, Trash2Icon, TypeIcon } from "lucide-react"
...
export function FlowFlowNodeToolbar({ toolbarOptions, visible = false }: { toolbarOptions?: ToolbarOptions, visible: boolean }) {
  const { t } = useTranslate()

  const options = { ...FlowFlowNodeToolbarOptions, ...toolbarOptions }

  const configs: Record<ToolbarOptionKey, { icon: ReactElement, label: string, onClick: () => void }> = {
    previewBtn: {
      icon: <EyeIcon />,
            ...
            return options[option as ToolbarOptionKey] ?
              <Tooltip key={option}>
                <TooltipTrigger asChild>
                  <Button variant="ghost" size="xs" onClick={configs[option as ToolbarOptionKey].onClick}>
